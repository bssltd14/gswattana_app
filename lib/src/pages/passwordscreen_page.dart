...
class _PasswordScreenPageState extends State<PasswordScreenPage> {
  ...
  Future<Null> validateThaiId(StateSetter updateState) async {
    ...
    if (_pinThaiIdController.text.length > 0 &&
        _pinThaiIdController.text == Constant.CUSTTHAIIDTEMP) {
      updateState(() {
        isThaiIdValid = true;
      });
    } else {
  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
      ...
      appBar: AppBar(
        title: Text(
          Constant.MOBILEAPPPASSWORDTEMP.length == 6
              ? "เข้าสู่ระบบ รหัสลูกค้า ${Constant.CUSTIDTEMP}"
              : "บันทึกรหัสผ่านใหม่ รหัสลูกค้า ${Constant.CUSTIDTEMP}",
      ...
      body: SingleChildScrollView(
        child:
          ...
          return Constant.MOBILEAPPPASSWORDTEMP.length == 6
              ? Column(
                  children: <Widget>[
                    Padding(
                    ...
                    InkWell(
                      onTap: () {
                        setState(() {
                          Constant.MOBILEAPPPASSWORDTEMP = "";
  ...
  void _SaveLogin() async {
    ...
    prefs.setString(Constant.MEMBERID_PREF, Constant.MEMBERID);
    prefs.setString(Constant.CUSTTHAIID_PREF, Constant.CUSTTHAIID);
  ...
  void checkPassword() async {
    print(
        "password:${Constant.MOBILEAPPPASSWORDTEMP} password input:${_pinPasswordController.text}");
    if (Constant.MOBILEAPPPASSWORDTEMP == _pinPasswordController.text) {
      Constant.IS_LOGIN = true;
      Constant.CUSTID = Constant.CUSTIDTEMP;
      Constant.CUSTNAME = Constant.CUSTNAMETEMP;
      Constant.CUSTTEL = Constant.CUSTTELTEMP;
      Constant.MEMBERID = Constant.MEMBERIDTEMP;
      Constant.CUSTTHAIID = Constant.CUSTTHAIIDTEMP;
      _SaveLogin();
  ...
  Future ResetPassword(String mobileAppPassword) async {
    try {
      ...
      final url =
          "${Constant.API}/customer/resetpassword?searchCustTel=${Constant.CUSTTELTEMP}&searchCustThaiId=${Constant.CUSTTHAIIDTEMP}&searchMobileAppPassword=${mobileAppPassword}";
  ...
  void showDialogNotCancel() {
    showDialog(
      context: Constant.scaffoldKey.currentContext,
